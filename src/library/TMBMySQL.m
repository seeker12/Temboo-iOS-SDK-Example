 ...
 */
@implementation TMBMySQL_XMLToDB_Inputs
	...
	*(optional, integer) The database port. Defaults to 3306.
	 */
 ...
 */
@implementation TMBMySQL_XMLToDB_ResultSet
 ...
 */
@implementation TMBMySQL_XMLToDB
	 ...
	 *
	 * @return - The initialized TMBMySQL_XMLToDB Choreo
	 */
	-(id)initWithSession:(TMBTembooSession *)session {
		TMBTembooUri *uri = [[[TMBTembooUri alloc] initWithString:@"/Library/MySQL/XMLToDB"] autorelease];
	...
	-(void)choreographyDidFinishExecuting:(NSDictionary*)response {
		TMBMySQL_XMLToDB_ResultSet *results = [[[TMBMySQL_XMLToDB_ResultSet alloc] initWithResponse:response] autorelease];
	...
	-(TMBMySQL_XMLToDB_Inputs*)newInputSet {
		return [[[TMBMySQL_XMLToDB_Inputs alloc] init] autorelease];
	}
@end
 ...
 */
@implementation TMBMySQL_RunCommand_Inputs
	...
	*(optional, string) The preferred format for the database results. Accepted formats are json (the default) and xml. This input only applies when providing a SELECT, SHOW, or DESCRIBE statement for the SQL input.
	 */
 ...
 */
@implementation TMBMySQL_RunCommand_ResultSet
	 ...
	 * @return - NSString* The data returned from the database. This output will only contain a value when a SELECT, SHOW, or DESCRIBE statement is provided. Results are returned as JSON or XML depending on the ResponseFormat.
	 * @throws TMBExceptionNotFound if output does not exist. (Note an empty response is considered valid)
	 */
 ...
 */
@implementation TMBMySQL_RunCommand
	 ...
	 * @param session - A TMBTembooSession containing valid Temboo Application Key credentials
	 *
	 * @return - The initialized TMBMySQL_RunCommand Choreo
	 */
	-(id)initWithSession:(TMBTembooSession *)session {
		TMBTembooUri *uri = [[[TMBTembooUri alloc] initWithString:@"/Library/MySQL/RunCommand"] autorelease];
		return [super initWithSession:session andUri:uri];
	}	
	 ...
	 */
	-(void)choreographyDidFinishExecuting:(NSDictionary*)response {
		TMBMySQL_RunCommand_ResultSet *results = [[[TMBMySQL_RunCommand_ResultSet alloc] initWithResponse:response] autorelease];
		[self.choreoDelegate choreographyDidFinishExecuting:results];
	}
	...
	-(TMBMySQL_RunCommand_Inputs*)newInputSet {
		return [[[TMBMySQL_RunCommand_Inputs alloc] init] autorelease];
	}
@end
 ...
 */
@implementation TMBMySQL_JSONToDB_Inputs
 ...
 */
@implementation TMBMySQL_JSONToDB_ResultSet
 ...
 */
@implementation TMBMySQL_JSONToDB
	 ...
	 * @param session - A TMBTembooSession containing valid Temboo Application Key credentials
	 *
	 * @return - The initialized TMBMySQL_JSONToDB Choreo
	 */
	-(id)initWithSession:(TMBTembooSession *)session {
		TMBTembooUri *uri = [[[TMBTembooUri alloc] initWithString:@"/Library/MySQL/JSONToDB"] autorelease];
		return [super initWithSession:session andUri:uri];
	}	
	 ...
	 */
	-(void)choreographyDidFinishExecuting:(NSDictionary*)response {
		TMBMySQL_JSONToDB_ResultSet *results = [[[TMBMySQL_JSONToDB_ResultSet alloc] initWithResponse:response] autorelease];
		[self.choreoDelegate choreographyDidFinishExecuting:results];
	}
	...
	-(TMBMySQL_JSONToDB_Inputs*)newInputSet {
		return [[[TMBMySQL_JSONToDB_Inputs alloc] init] autorelease];
	}
@end
